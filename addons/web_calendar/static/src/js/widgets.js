...
var QuickCreate = Dialog.extend({
	...
	var self = this;
	$('#gvm_project option').remove();
	$('#gvm_project').append('<option id="0" value="0"></option>');
	var Project = new Model('project.project');
	Project.query(['name'],['id']).all().then(function(id){
	  $.each(id, function(index, item){
	    $('#gvm_project').append('<option id="'+index+'" value="'+item.id+'">'+item.name+'</option>');
	    });
	  });
    },
    get_title: function () {
    ...
    quick_add: function() {
        ...
        var val2 = parseInt(this.$('#gvm_project').val().trim());
        return (val)? this.quick_create({'name': val,'project_id': val2}) : false;
    },
